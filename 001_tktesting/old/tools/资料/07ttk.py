...
#win.iconbitmap(r"C:\Users\Administrator\pyprojects\PyInstallerUse\QQ.ico")
...
def radCall():
    radSel=radVar.get()
    if   radSel == 0: tttt.set(colors[0])  # zero-based
    elif radSel == 1: tttt.set(colors[1])  # using list
    elif radSel == 2: tttt.set(colors[2])
...
ttk.Label(buttons_frame, text="Label2").grid(column=2, row=0, sticky=tk.W)
ttk.Label(buttons_frame, text="Label3").grid(column=4, row=0, sticky=tk.W)
tttt=tk.StringVar()
tttt.set("Waiting...")
vvv=ttk.Button(buttons_frame, textvariable=tttt)
vvv.grid(column=6, row=0, sticky=tk.W)
